...
func TestNewSystem(t *testing.T) {
	system, err := NewSystem(DbEntity{ID: 111, DbName: "system of a down"}, Coordinate{X: 1.0, Y: 2.0, Z: 3.0}, true)
	require.Nil(t, err)
	...
	assert.True(t, system.NeedsPermit)
	assert.Empty(t, system.Facilities)
}

func TestSystem_Distance(t *testing.T) {
	first, err := NewSystem(DbEntity{ID: 123, DbName: "first"}, Coordinate{X: 1.0, Y: -2.0, Z: 5.0}, false)
	require.Nil(t, err)
	assert.Equal(t, Square(0), first.Distance(first))

	second, err := NewSystem(DbEntity{ID: 201, DbName: "second"}, Coordinate{Y: 3.3, Z: 9.9}, false)
	require.Nil(t, err)
...
func TestSystem_Name(t *testing.T) {
	system, err := NewSystem(DbEntity{ID: 100, DbName: "System1"}, Coordinate{}, false)
	require.Nil(t, err)
